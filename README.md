...
A small PHP CLI tool that talks to the [TheTVDB v4 API](https://thetvdb.github.io/v4-api/) and fetches poster artwork for your TV and movie library. Point it at a folder of TV shows or movies — `--movie` switches it to movie mode — and it matches each folder to a series or movie, downloads the poster, and drops `poster.jpg` (or `poster.png`) into the folder, the convention Plex, Kodi, and Jellyfin all understand. `--seasons` fetches posters for `Season N` / `Specials` folders too, `--clean` tidies the folder names up as it goes, and a title [TheTVDB](https://thetvdb.com) has no record of falls back to a [TMDB](https://www.themoviedb.org) lookup.

## Requirements

- PHP 8.x CLI with the `curl` extension enabled (developed on PHP 8.5, Windows + Git Bash)
- A [TheTVDB](https://thetvdb.com) v4 API key
- A [TMDB](https://www.themoviedb.org) v3 API key (Optional for fallback functionality)
...
1. Create a `.env` file in the project folder containing your API key:
   ...
   Optionally add `TMDB_API_KEY=...` (a [TMDB](https://www.themoviedb.org) v3 API key) to enable the fallback lookup for titles TheTVDB has no record of — see *Fetch posters for a library* below.
...
Search TMDB movie: php run.php --title="Interstellar" --movie --tmdb
```
...
When TVDB has no record of the title at all, the folder is looked up on [TMDB](https://www.themoviedb.org) instead and its poster is used — the `Matched:` and `Done   :` lines then carry a `tmdb-` id (`(tmdb-movie-1234567)`, `(tmdb-tv-12345)`) so it is obvious where the poster came from, and the TMDB notice is printed once per run. The same don't-guess rule applies on this side: a TMDB result has to match the folder's title, so a folder name TVDB merely failed to parse is still reported rather than guessed at. When TMDB has nothing either, the skip line and the table of TVDB titles appear exactly as before. Add `TMDB_API_KEY` to `.env` to enable this; without it the tool is TVDB-only.
...
With `--posters --clean`, a folder that only TMDB could name still gets its poster and the rest of the tidy-up, but the folder itself is never renamed on a TMDB match — the `Clean  :` line says so. TMDB is asked for its English title, so renaming on it would turn a foreign-named folder English, which the TVDB path deliberately avoids. On its own, `--clean` downloads nothing and so makes no TMDB lookups.
